...
class MdDocument {
  ...
  addParagraphElement(after) {
    ...
    if (after === undefined) {
      ...
      const index = this.elements.findIndex(element => element.id === after);
...
class MdView {
  ...
  paragraphElementAdded({ id, text, after }) {
    ...
    input.oninput = e => {
      ...
      while (input.clientHeight != input.scrollHeight) {
        input.rows = input.rows + (input.clientHeight < input.scrollHeight ? 1 : (-1));
      }
    };
    input.onkeypress = e => {
      switch (e.key) {
